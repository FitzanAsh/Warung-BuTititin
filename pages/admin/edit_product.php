...
// Fungsi untuk menyimpan perubahan produk ke dalam database
function editProduk($id, $gambar, $judul, $harga, $deskripsi, $kategori)
{
    $koneksi = new mysqli("localhost", "root", "", "warungbutitin");
    ...
    $query = "UPDATE Products SET image = ?, name = ?, price = ?, description = ?, category_id = ? WHERE id = ?";
    $stmt = $koneksi->prepare($query);
    $stmt->bind_param("ssdsii", $gambar, $judul, $harga, $deskripsi, $kategori, $id);

    if ($stmt->execute()) {
        header("Location: admin.php");
        exit;
    } else {
        echo "Error: Gagal menyimpan perubahan produk. " . $stmt->error;
    }
...
// Cek apakah produk dengan ID tersebut ditemukan
if ($produk === null) {
    echo "Produk tidak ditemukan.";
    exit;
}

// Proses form jika tombol "Simpan Perubahan" ditekan
if (isset($_POST['submit'])) {
    ...
    editProduk($id_produk, $gambar, $judul, $harga, $deskripsi, $kategori);
}
?>
...
<a href="../admin/pelanggan.php" class="pelanggan-product-btn">
    ...
    <form action="" method="POST" enctype="multipart/form-data">
        ...
        <input type="text" name="judul" id="judul" value="<?php echo $produk['name']; ?>" required>

        <label for="harga">Harga:</label>
        <input type="number" name="harga" id="harga" value="<?php echo $produk['price']; ?>" required>

        <label for="deskripsi">Deskripsi:</label><br>
        <textarea name="deskripsi" id="deskripsi" rows="4" required><?php echo $produk['description']; ?></textarea>

        <label for="kategori">Kategori:</label>
        <select name="kategori" id="kategori" required>
            <option value="1" <?php if ($produk['category_id'] == 1) echo 'selected'; ?>>Makanan</option>
            <option value="2" <?php if ($produk['category_id'] == 2) echo 'selected'; ?>>Minuman</option>
        </select>
